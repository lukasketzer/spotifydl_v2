...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "link",
        help="The link of the spotify playlist or album. The playlist should be public.",
    )
    parser.add_argument(
        "-t",
        "--thread_count",
        default=16,
        help="Amout of threads used by the script",
        type=int,
    )
    parser.add_argument(
        "-f",
        "--fix_missing",
        action="store_true",
        help="""If flag is set, already downloaded songs will not be replaced but skipped. Only missing songs will
